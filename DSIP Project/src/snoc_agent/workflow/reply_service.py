...
def create_outbound_message(
    ...
    *,
    ...
    subject = reject_header_injection(subject)
    domain = sender.rsplit("@", 1)[-1] if "@" in sender else None
    rfc_message_id = make_msgid(domain=domain)
    references = build_references(source_email.references_json, source_email.rfc_message_id)
    headers = {
        "Message-ID": rfc_message_id,
        "In-Reply-To": normalize_message_id(source_email.rfc_message_id) or "",
    ...
    outbound = EmailMessage(
        ...
        subject=subject,
        normalized_subject=source_email.normalized_subject,
        raw_text=body,
