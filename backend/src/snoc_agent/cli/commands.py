...
from snoc_agent.ai.model_registry import ModelPair, canonical_hf_model_id
...
def models_check(settings: Settings, *, refresh: bool) -> None:
    if settings.effective_llm_provider != LLMProvider.HUGGINGFACE:
        raise ValueError("models check requires LLM_PROVIDER=huggingface")
    ...
    try:
        ...
        probe = getattr(backend, "probe_chat", None)
        ...
        try:
            structured_result = backend.generate_structured(
...
def models_smoke_test(
    settings: Settings, *, analyzer_model: str, verifier_model: str, output_dir: Path
) -> None:
    print_json(
        run_hf_smoke_test(
            settings,
...
def _real_orchestrator(settings: Settings) -> tuple[Any, MailOrchestrator]:
    ...
    mailbox = RealIMAPMailbox(
        ...
        use_ssl=settings.effective_imap_ssl,
