...
class OutboxService:
    ...
    def send_once(self, limit: int = 100) -> tuple[int, int]:
        with session_scope(self.session_factory) as session:
            ids = [message.id for message in OutboxRepository(session).claim_pending(limit)]
        sent = 0
        failed = 0
        for outbox_id in ids:
            with session_scope(self.session_factory) as session:
                outbox = session.get(OutboxMessage, outbox_id)
                if outbox is None or outbox.status != OutboxStatus.SENDING.value:
                    continue
                ...
                if result.accepted:
                    ...
                    if not result.transient_failure or outbox.retry_count >= self.max_retries:
                        ...
                        outbox.dead_lettered_at = utc_now()
                    else:
                        from snoc_agent.db.base import utc_now

                        outbox.status = OutboxStatus.PENDING.value
                        outbox.claimed_at = None
                        delay_seconds = min(3600, 30 * (2 ** (outbox.retry_count - 1)))
                        outbox.next_attempt_at = utc_now() + timedelta(seconds=delay_seconds)
                    failed += 1
        return sent, failed
