...
class DecisionContext:
    ...
    enforce_evidence_provenance: bool = False
    ...
    def decide(
        ...
        *,
        ...
        semantic_fields_supported = verification.pdv_supported == "yes" and (
            verification.phone_supported == "yes"
            if action in {OperationAction.VPN_ACCESS, OperationAction.OTP_NUMBER_CHANGE}
            else verification.phone_supported == "not_required"
        )
        agreement = (
            ...
            and verification.local_operation_id in {None, proposal.local_operation_id}
        )
        contradiction = (
            analysis.contradiction_with_stored_state
            or verification.contradiction_present
            or verification.stored_state_compatible == "no"
        )
        ...
        ambiguous = bool(proposal.ambiguity_reasons or scoped_analysis_ambiguity or major_ambiguity)
        missing = set(proposal.missing_fields) | set(verification.missing_fields)
        hard_invariants = {
            ...
            "verifier_operation_scope_matches": verification.local_operation_id
            in {None, proposal.local_operation_id},
        }
        ...
        if verification.local_operation_id not in {None, proposal.local_operation_id}:
            reasons.append("verifier_operation_scope_mismatch")
        ...
        if missing or not field_check.passed:
            ...
            if (
                ...
                and not major_ambiguity
                and present_required_evidence_allowed
                and configured_confidence_thresholds_passed
                and not unexpected_additional_fields
                and additional_fields_evidence_supported
                and proposal_matches_stored_action
