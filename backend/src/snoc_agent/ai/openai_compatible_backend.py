...
class OpenAICompatibleBackend:
    def __init__(
        ...
        *,
        ...
        if not endpoint.startswith("/") or "://" in endpoint:
            raise ValueError("endpoint must be an absolute-path reference")
    ...
    def _post(
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                response = self.client.post(f"{self.base_url}{self.endpoint}", json=payload)
                ...
                if not response.is_error:
                    if len(response.content) > self.max_response_bytes:
                        raise InferenceError(
                            ...
                            "inference provider response exceeded the configured size limit",
    ...
    def health_check(self) -> bool:
        """Return provider reachability without sending prompts or exposing credentials."""
    ...
    def list_models(self) -> list[str]:
        """Optional, bounded diagnostic for OpenAI-compatible model catalogs."""
