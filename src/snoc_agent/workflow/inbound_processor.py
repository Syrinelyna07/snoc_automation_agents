    ...
    def _materialize_analysis(
        ...
        with session_scope(self.session_factory) as session:
            ...
            if prepared.clarification_id:
                ...
                if not proposals_remaining and len(targets) == 1:
                    deterministic_proposal = self._single_field_clarification_proposal(
                        prepared,
                        targets[0],
                    )
                    if deterministic_proposal is not None:
                        proposals_remaining.append(deterministic_proposal)
                        deterministic_proposal_ids.add(
                            deterministic_proposal.local_operation_id
                        )
