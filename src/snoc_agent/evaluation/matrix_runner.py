...
def run_persistent_matrix(
    ...
    *,
    ...
    if settings.effective_llm_provider == LLMProvider.HUGGINGFACE:
        endpoint_identity = settings.effective_hf_base_url
    elif settings.effective_llm_provider == LLMProvider.VLLM:
        endpoint_identity = [
            {
    ...
    configuration = {
        ...
        "provider_policy": (
            settings.hf_provider_policy.value
            if settings.effective_llm_provider == LLMProvider.HUGGINGFACE
            else None
        ),
        "analyzer_provider": (
            ...
            if settings.effective_llm_provider == LLMProvider.HUGGINGFACE
            else settings.vllm_analyzer_deployment.value
            if use_vllm
            else None
        ),
        "verifier_provider": (
            ...
            if settings.effective_llm_provider == LLMProvider.HUGGINGFACE
            else settings.vllm_verifier_deployment.value
            if use_vllm
            else None
        ),
        "routing_suffix_enabled": (
        ...
        "generation": {
            ...
            "analyzer_max_tokens": (
                ...
                if use_vllm
                else settings.hf_max_output_tokens_analyzer
            ),
            "verifier_max_tokens": (
                settings.vllm_max_output_tokens_verifier
                if use_vllm
                else settings.hf_max_output_tokens_verifier
            ),
            "max_retries": settings.vllm_max_retries if use_vllm else settings.hf_max_retries,
            "retry_base_seconds": (
                settings.vllm_retry_base_seconds if use_vllm else settings.hf_retry_base_seconds
            ),
            "supports_logprobs": settings.llm_supports_logprobs,
            "json_schema": (
                settings.vllm_use_json_schema if use_vllm else settings.hf_use_json_schema
            ),
            "json_object_fallback": (
                settings.vllm_allow_json_object_fallback
                if use_vllm
                else settings.hf_allow_json_object_fallback
            ),
            "prompt_json_fallback": (
                settings.vllm_allow_prompt_json_fallback
                if use_vllm
                else settings.hf_allow_prompt_json_fallback
            ),
            "extra_body_hash": canonical_hash({} if use_vllm else settings.hf_extra_body),
